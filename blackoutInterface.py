...
while running:
    ...
    # Calculate listener position
    player_position = (player_x, player_y)
    
    # Calculate distance and direction from listener to object
    object_position = (train1_x, train1_y)
    distance = np.linalg.norm(np.array(player_position) - np.array(object_position))
    direction = (np.array(object_position) - np.array(player_position))  #/ np.linalg.norm(distance)
    
    #Calculate HRTF and get the audio
    audioProcessing()
